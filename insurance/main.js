...
class TokenMain extends Contract {
  ...
  constructor(data) {
    ...
    this._object = new Object(data)
  }
  //---------------------Object------------------------------
  async Customer() {
    let Customer = await this._object.createObject('CUSTOMER')
    return Customer
  }
  get_Customer() {
    let Customer = this._object.getObjectByType('CUSTOMER')
    return Customer
  }
  async Company() {
    let Company = await this._object.createObject('COMPANY')
    return Company
  }
  get_Company() {
    let Company = this._object.getObjectByType('COMPANY')
    return Company
  }
  // --------------------Fill_in_forms---------------------------
  ...
  async Fill_in_forms() {
    await this._object.checkObject(this.sender, 'CUSTOMER')
  ...
  checkProcess1(address) {
    this.check_Pay_insurance_fee = this.getProcessByAddress(address);
    ...
    else if (this.check_Renew_insurance.type == 'RENEW_INSURACE') {
      return true;
    }
    else {
      throw `PAY_INSURANCE_FEE_OR_FILL_IN_FORMS_OR_RENEW_INSURACE_FOR_CHECK IS NOT EXIST`;
    }
  }
  async Pay_insurance_fee_or_Fill_in_forms_or_Renew_insurance() {
    await this.checkProcess1(this.sender, 'PAY_INSURANCE_FEE_OR_FILL_IN_FORMS_OR_RENEW_INSURACE_FOR_CHECK')
    let check = await this._process.createProcess('PAY_INSURANCE_FEE_OR_FILL_IN_FORMS_OR_RENEW_INSURACE')
    return check
  }

  get_Pay_insurance_fee_or_Fill_in_forms_or_Renew_insurance() {
    return this._process.getProcessByType('PAY_INSURANCE_FEE_OR_FILL_IN_FORMS_OR_RENEW_INSURACE')
  }
  ...
  async Someone_buys_insurance(address_Pay_insurance_fee_or_Fill_in_forms_or_Renew_insurance) {
    this._object.checkObject(this.sender, 'COMPANY')
    ...
    if (!check_Pay_insurance_fee_or_Fill_in_forms_or_Renew_insurance || check_Pay_insurance_fee_or_Fill_in_forms_or_Renew_insurance.type !== 'PAY_INSURANCE_FEE_OR_FILL_IN_FORMS_OR_RENEW_INSURACE')
      throw 'PAY_INSURANCE_FEE_OR_FILL_IN_FORMS_OR_RENEW_INSURACE IS NOT EXIST'
  ...
  async Issue_the_receipt() {
    await this._object.checkObject(this.sender, 'COMPANY')
  ...
  checkProcess2(address) {
    ...
    this.check_Issue_the_receipt = this.get_Issue_the_acceipt(address);
  ...
  getThe_insurance_company_pays_compensation() {
